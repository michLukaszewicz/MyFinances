...
import type { ClientTransaction } from "../../Pages/App/HomePage";
...
const onSubmit = (event: React.FormEvent<HTMLFormElement>) => {
  ...
  onAddTransaction({...form, id: -1, date: new Date(form.date)})
}
      ...
      <Box header="Add Transaction">
        <form className="p-3 grid grid-cols-8 items-center gap-3" onSubmit={onSubmit}>
          <div className="flex flex-col gap-2 mb-3">
            <label htmlFor="otherSideOfTransaction">{form.amount < 0 ? "To" : "From"}</label>
            <input
              autoFocus
              type="text"
